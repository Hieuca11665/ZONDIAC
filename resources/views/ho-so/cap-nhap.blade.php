        ...
        <div class="col-lg-8 col-md-7">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                ...
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="{{ route('cap-nhat-ho-so') }}" enctype="multipart/form-data" class="row g-4">
                        @csrf
    ...
    if (cameraButton && fileInput && formFileInput) {
        ...
        fileInput.addEventListener('change', function() {
            if (this.files.length > 0) {
                // Reset giá trị của input chính trước khi chuyển file mới
                formFileInput.value = '';
                ...
                // Reset giá trị của input ẩn để có thể chọn lại cùng một file
                this.value = '';
            }
        });

        // Thêm xử lý cho input chính
        formFileInput.addEventListener('change', function() {
            if (this.files.length > 0) {
                // Hiển thị preview tương tự như trên
                const reader = new FileReader();
                reader.onload = function(e) {
                    const profileImg = document.querySelector('.profile-img');
                    if (profileImg && profileImg.tagName === 'IMG') {
